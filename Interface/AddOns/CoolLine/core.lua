	...
	if not CoolLineDD then
		...
		CoolLineDD.initialize = function(self, lvl)
			if lvl == 1 then
				...
				AddList(lvl, "边框插入", "borderinset") -- Added Options
				AddColor(lvl, "边框颜色", "bordercolor")
				AddList(lvl, "字体", "font")
				AddColor(lvl, "字体颜色", "fontcolor")
				AddList(lvl, "字体大小", "fontsize")
				AddColor(lvl, "法术颜色", "spellcolor")
				AddColor(lvl, "物品/物品颜色", "nospellcolor")
				AddList(lvl, "非活动不透明度", "inactivealpha")
				AddList(lvl, "活动不透明度", "activealpha")
			...
			elseif lvl and lvl > 1 then
				...
				elseif sub == "More" then
					AddToggle(lvl, "垂直", "vertical")
					AddToggle(lvl, "反转", "reverse")
					AddToggle(lvl, "禁用施法失败", "hidefail")
					AddToggle(lvl, "禁用装备", "hideinv")
					AddToggle(lvl, "禁用背包", "hidebag")
					AddToggle(lvl, "禁用宠物", "hidepet")
					AddToggle(lvl, "按角色保存", "perchar")
