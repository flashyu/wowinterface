...
local SHAMAN_FORM = 136095; -- spirit wolf
...
function TryAndDismount(msg, self)
    if (msg and tContains(ErrorsToCheckFor, msg)) then
        ...
        for i = 1, TOTAL_BUFFS do
            ...
            if (currBuffTex) then
                if (currBuffTex == SHAMAN_FORM) then
                    if (not InCombatLockdown()) then
                        CancelUnitBuff("player", i);
                    end
                    return;
                elseif (tContains(BuffsToCheckFor, currBuffTex)) then
                ...
                -- exit as we reached the end of active buffs
                return;
            end
        end
    end
